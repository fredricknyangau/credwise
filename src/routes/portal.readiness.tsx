...
export const Route = createFileRoute("/portal/readiness")({
  head: () => ({ meta: [{ title: "Credit readiness - CredWise" }] }),
  component: Readiness,
});
...
function Readiness() {
  ...
  if (!data)
    return <div className="grid place-items-center py-20 text-muted-foreground">Loading…</div>;
    ...
    <main className="mx-auto max-w-3xl px-4 py-6">
        ...
        <div className="relative z-10 grid gap-8 md:grid-cols-2 md:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-brand-secondary">
              Ethical credit readiness
            </p>
            <h1 className="mt-2 font-display text-3xl font-bold">Your score, explained</h1>
            <p className="mt-3 text-sm text-muted-foreground">
              Calculated from financial behavior, literacy progress, and community ties - not income
              alone.
            </p>
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="mt-6 text-center text-sm text-muted-foreground">
              You are{" "}
              <span className="font-bold text-background underline decoration-brand-secondary decoration-2 underline-offset-4">
                Ready for Credit
              </span>
            </p>
          </div>
      ...
      <section className="mt-8 rounded-2xl border border-border bg-brand-primary/5 p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          ...
          CredWise replaces that with verifiable behavior - savings consistency, financial literacy,
          and cooperative participation. Every factor that affects your score is shown above and can
          be improved through your own actions. No hidden inputs.
        </p>
